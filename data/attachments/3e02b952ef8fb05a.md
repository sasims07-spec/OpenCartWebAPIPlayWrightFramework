...
- Name: search.spec.ts >> footer exist on product page
- Location: tests/search.spec.ts:49:1
...
```ts
  ...
  6  |   await loginPage.doLogin(process.env.OC_USERNAME!, process.env.OC_PASSWORD!);
  7  | });
  8  | 
  9  | /*test("verify search results count", async ({ homePage, searchResultsPage }) => {
  10 |   await homePage.doSearch("macBook");
  11 |   expect(await searchResultsPage.getSearchResultsCount()).toBe(3);
  12 | });
  13 | 
  14 | test("verify user is able to land on the product page", async ({
  15 |   homePage,
  16 |   searchResultsPage,
  17 |   page,
  18 | }) => {
  ...
  22 | }); */
  23 | 
  24 | const productData = CsvHelper.readCsv("src/data/product.csv");
  25 | for (const row of productData) {
  26 |   test(`verify search results count - ${row.searchkey} ${row.productname}`, async ({ homePage, searchResultsPage }) => {
  27 |     await homePage.doSearch(row.searchkey);
  28 |     expect(await searchResultsPage.getSearchResultsCount()).toBe(Number(row.resultcount));
  29 |   });
  30 | }
  31 | 
  32 | for (const row of productData) {
  33 |   if (Number(row.resultcount) === 0) continue;
  34 |   test(`verify user is able to land on the product page - ${row.searchkey} ${row.productname}`, async ({
  35 |     homePage,
  36 |     searchResultsPage,
  37 |     page,
  38 |   }) => {
  39 |     await homePage.doSearch(row.searchkey);
  40 |     await searchResultsPage.selectProduct(row.productname);
  41 |     expect(await page.title()).toBe(row.productname);
  42 |   });
  43 | }
  44 | 
  45 | test("comp logo exists on product page", async ({ basePage }) => {
  46 |   expect(await basePage.isLogoVisible()).toBeTruthy();
  47 | });
  48 | 
  49 | test("footer exist on product page", async ({ basePage }) => {
> 50 |   expect(await basePage.getPageFootersCount()).toBe(16);
     |                                                ^ Error: expect(received).toBe(expected) // Object.is equality
  51 | });
  52 | 
```